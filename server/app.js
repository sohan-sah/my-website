...
/* =========================================================
   2b) PHOTO COLORIZE   POST /api/colorize-image
   multipart/form-data field: "image"

   NOT CONFIGURED: colorizing black & white photos needs a
   dedicated hosted model, and none is wired up on the free HF
   tier this backend runs on. Rather than fake a result, this
   returns a clear, honest error so the frontend can show a
   real "not configured" message with Retry / Try another file
   — see README "Swapping models" to wire up a real one.
   ========================================================= */
app.post("/api/colorize-image", upload.single("image"), async (req, res) => {
  if (!req.file) return res.status(400).json({ error: "No image uploaded" });
  res.status(501).json({
    error: "Photo colorization needs a hosted AI model that isn't configured yet on this backend. See server/app.js (/api/colorize-image) to wire one up.",
  });
});
  ...
  try {
    ...
    if (currentLongEdge && longEdge > currentLongEdge) {
      ...
      pipeline = pipeline.resize({
        ...(isLandscape ? { width: longEdge } : { height: longEdge }),
