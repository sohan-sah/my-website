...
## 4. Swapping / adding models

Each feature now tries a **chain** of candidate models instead of a
single hardcoded one, because Hugging Face's free `hf-inference`
provider adds and drops model coverage over time — the fixed model id
that worked today can start returning `Model not supported by provider
hf-inference` next month with no warning. `server/app.js` tries your
env var first, then falls through a short list of widely-mirrored
fallback checkpoints automatically. A genuine error (bad token,
malformed request) is never silently retried — only "this specific
model isn't available here" failures fall through to the next
candidate.
...
HF_MODEL_UPSCALE=stabilityai/stable-diffusion-x4-upscaler
```
It's tried first, then the built-in fallback list in `server/app.js`
kicks in if it's unset or no longer served.
...
provider for the task you need (text-to-image, image-to-image,
image-segmentation). Then either set the matching `HF_MODEL_*` env
var, or add the id to that feature's fallback array in
`server/app.js`. Full provider docs:
